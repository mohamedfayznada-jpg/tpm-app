...
// Architected by: Eng. Mohamed Fayez
// ==========================================
...
firebase.auth().onAuthStateChanged(async user => {
    ...
    if (user) {
        ...
        departments = dSnap.val() || ['إنتاج', 'صيانة', 'حقن الكابينة'];
...
async function login() {
    ...
    try { await firebase.auth().signInWithEmailAndPassword(username + "@tpm.app", password); } catch (e) { showToast('بيانات الدخول غير صحيحة'); document.getElementById('cloudStatus').innerHTML = "غير متصل"; }
}
...
async function signup() {
    ...
    try {
        ...
        const newUserObj = {
            name: fullName, username: user, requestedRole: requestedRole,
        ...
        if (sessionScannedBarcodes.has(decodedText)) {
            ...
            document.getElementById('searchResults').innerHTML = `
                <div style="padding:20px; background:rgba(198,40,40,0.1); border:1px solid var(--danger); border-radius:15px; text-align:center;">
                    ...
                    <div style="margin-top:10px; font-size:12px; color:var(--text-muted);">${decodedText}</div>
...
async function uploadImageToStorage(fileOrDataUrl) {
    try {
        ...
        const response = await fetch('/api/imgbb', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ image: b64 }) });
        const data = await response.json();
        if(data.success) return data.data.url;
        return null;
    } catch(e) { console.error("ImgBB Error:", e); showToast('⚠️ فشل الرفع عبر السيرفر'); return null; }
}
...
function toggleSidebar() {
    const sb = document.getElementById('mainSidebar'); const ov = document.getElementById('sidebarOverlay');
    if(!sb) return;
...
window.addEventListener('DOMContentLoaded', () => { if(localStorage.getItem('tpm_theme') === 'light') { document.body.classList.add('light-theme'); } });

// ==========================================
// 🎖️ محرك التقييم ولوحة الصدارة
// ==========================================
...
window.renderProfileAndSettings = function() {
    ...
    const uid = firebase.auth().currentUser ? firebase.auth().currentUser.uid : null; let u = usersData[uid] || currentUser;
    const nameEl = document.getElementById('profileName'); if(nameEl) nameEl.innerText = u.name || 'مستخدم مجهول';
    const roleEl = document.getElementById('profileRoleBadge'); if(roleEl) { let roleName = u.role === 'admin' ? 'مدير المصنع (Admin)' : (u.role === 'auditor' ? 'مراجع TPM' : 'فني صيانة'); roleEl.innerText = `الرتبة: ${roleName}`; }
    const avatarEl = document.getElementById('profileAvatar'); if(avatarEl) avatarEl.src = u.avatar || `https://ui-avatars.com/api/?name=${u.name || 'User'}&background=1E3A8A&color=ffffff`;
    ...
    document.querySelectorAll('.btn-role-admin').forEach(el => { el.style.display = u.role === 'admin' ? 'inline-block' : 'none'; });
    if(typeof switchSettingsTab === 'function') { switchSettingsTab('my-activity'); }
};
...
window.openMyFullProfile = function() { renderProfileAndSettings(); showToast('تم فتح مركز القيادة بنجاح 🛡️'); };
...
window.updateProfilePic = async function(event) {
    ...
    try {
        processAndEnhanceImage(file, async function(dataUrl) {
            ...
            if(url) {
                const uid = firebase.auth().currentUser.uid; await db.ref(`tpm_system/users/${uid}`).update({ avatar: url });
                document.getElementById('profileAvatar').src = url; showToast('تم تحديث الصورة ✅');
            } else { showToast('⚠️ فشل رفع الصورة'); }
        });
    } catch(e) { console.error(e); showToast('حدث خطأ أثناء الرفع'); }
};
...
async function savePersonalData() {
    ...
    showToast('تم تحديث بياناتك بنجاح ✅'); renderProfileAndSettings(); showScreen('settingsScreen');
}

function saveApiKeys() {
    globalApiKeys.imgbb = document.getElementById('imgbbKeyInput').value.trim(); globalApiKeys.gemini = document.getElementById('geminiKeyInput').value.trim();
...
const originalRenderUserManagement = window.renderUserManagement;
...
function canAccess(screenId, action = 'view') {
    ...
    const uid = firebase.auth().currentUser ? firebase.auth().currentUser.uid : null; if (!uid) return false;
    const uData = usersData[uid];
    if (!uData || typeof uData === 'string') { return screenId === 'homeScreen' || screenId === 'tasksScreen' || screenId === 'tagsScreen'; }
    const userPerms = uData.permissions; if (!userPerms || !userPerms[screenId]) return false;
    if (action === 'edit') return userPerms[screenId] === 'edit';
    return userPerms[screenId] === 'view' || userPerms[screenId] === 'edit';
}

async function approveUser(uid) {
    const u = usersData[uid]; let finalPerms = u.permissions;
...
function deleteUser(uid) { if(confirm('هل أنت متأكد من حذف هذا المستخدم/الطلب نهائياً؟')) { db.ref('tpm_system/users/' + uid).remove(); showToast('تم حذف المستخدم بنجاح'); } }

let editingUserUid = null;
function openPermissionsModal(uid) {
    const u = usersData[uid]; if (!u || !u.permissions) return showToast('لا توجد أذونات قابلة للتعديل لهذا المستخدم (قديم)');
    editingUserUid = uid; const perms = u.permissions; const container = document.getElementById('permissionsContainer');
    const pages = { homeScreen: 'الرئيسية (Dashboard)', tasksScreen: 'المهام', historyScreen: 'التقارير (History)', kaizenScreen: 'كايزن', tagsScreen: 'التاجات', knowledgeScreen: 'عقل المصنع' };
...
async function saveUserPermissions() {
    ...
    let newPerms = {};
    pages.forEach(p => { let sel = document.getElementById('perm_' + p); if (sel) newPerms[p] = sel.value; });
    await db.ref(`tpm_system/users/${editingUserUid}/permissions`).set(newPerms);
    showToast('تم تحديث الأذونات بنجاح'); document.getElementById('permissionsModal').style.display = 'none';
}

// ==========================================
// 📋 محرك المهام (Kanban Tasks)
// ==========================================
...
// ==========================================
// 🏷️ التذاكر والتاجات (Tags & Abnormalities)
// ==========================================
...
async function addNewTag() {
    ...
    if (currentTagImg) {
        showToast('جاري رفع التاج والصورة... ⏳'); uploadedUrl = await uploadImageToStorage(currentTagImg);
        if(!uploadedUrl) showToast('⚠️ فشل رفع الصورة (تأكد من مفتاح ImgBB). سيتم حفظ التاج كنص فقط.');
    }
...
function submitManualKaizen() {
    ...
    imgBefore.onload = function() {
        imgAfter.onload = async function() {
            canvas.width = 600; canvas.height = 300; ctx.fillStyle = "#111d33"; ctx.fillRect(0,0,600,300);
            ...
            ctx.fillStyle = "rgba(198,40,40,0.85)"; ctx.fillRect(10, 10, 50, 25); ctx.fillStyle = "white"; ctx.font = "bold 14px Cairo"; ctx.fillText("قبل", 22, 27);
            ctx.fillStyle = "rgba(46,125,50,0.85)"; ctx.fillRect(540, 10, 50, 25); ctx.fillStyle = "white"; ctx.font = "bold 14px Cairo"; ctx.fillText("بعد", 552, 27);
            ...
            if (uploadedUrl) {
                ...
                document.getElementById('newKaizenTitle').value = ''; document.getElementById('kaizenBeforePreview').innerHTML = ''; document.getElementById('kaizenAfterPreview').innerHTML = '';
                kaizenImgs = { before: null, after: null }; document.getElementById('kaizenUploadModal').style.display = 'none';
...
// ==========================================
setInterval(() => { document.querySelectorAll('.sos-btn').forEach(btn => btn.remove()); }, 1000);
...
window.fetchGeminiAPI = async function(promptText, pdfBase64 = null) {
    ...
    try {
        ...
        if(!j.candidates || j.candidates.length === 0 || !j.candidates[0].content) throw new Error("جوجل رفضت الإجابة بسبب قيود الأمان. جرب صياغة أخرى.");
...
window.explainItem = async function(t) {
    ...
    } catch(e) { document.getElementById('aiModalText').innerHTML = `<div style="color:red; text-align:center; padding:20px;">⚠️ خطأ: ${e.message}</div>`; }
};
...
window.askFactoryAI = async function() {
    ...
    } catch(e) { document.getElementById('aiResponseText').innerHTML = `<b style="color:var(--danger);">⚠️ ${e.message}</b>`; }
};
...
window.handleMaterialUpload = function(event) {
    ...
    const reader = new FileReader();
    reader.onload = function(e) { tempBase64Pdf = e.target.result; document.getElementById('pdfExtractStatus').innerHTML = `✅ تم التجهيز: <b style="color:var(--primary);">${file.name}</b>`; };
    reader.readAsDataURL(file);
};
...
window.saveNewBook = async function() {
    ...
    let catEl = document.getElementById('kbCategory'); let cat = catEl ? catEl.value : 'TPM';
    ...
    document.getElementById('kbTitle').value = ''; document.getElementById('pdfExtractStatus').innerText = "اضغط هنا لرفع الكتالوج 📄"; tempBase64Pdf = null;
    showToast("✅ تم حفظ المرجع بنجاح!"); window.renderKnowledgeShelves();
};

window.renderKnowledgeShelves = function() {
    const container = document.getElementById('knowledgeListContainer'); if(!container) return;
    let kbArray = Array.isArray(knowledgeBaseData) ? knowledgeBaseData : Object.values(knowledgeBaseData || {});
    if(kbArray.length === 0) { container.innerHTML = '<div style="grid-column:1/-1; text-align:center; padding:40px; color:var(--text-muted); font-weight:bold;">لا توجد مراجع حالياً 📚</div>'; return; }
...
window.openBookDetail = async function(id) {
    let kbArray = Array.isArray(knowledgeBaseData) ? knowledgeBaseData : Object.values(knowledgeBaseData || {});
    let kb = kbArray.find(x => x.id == id); if(!kb) return;
    if(kb.hasPdf) {
        document.getElementById('aiModal').style.display = 'flex';
        ...
        try {
            ...
            if(snap.val() && snap.val().base64) {
                const b64 = snap.val().base64.split(',')[1] || snap.val().base64; const bin = atob(b64); const arr = new Uint8Array(bin.length);
...
window.deleteKnowledgeBook = async function(id) {
    if(confirm("⚠️ هل أنت متأكد من الحذف النهائي؟")) {
        let kbArray = Array.isArray(knowledgeBaseData) ? knowledgeBaseData : Object.values(knowledgeBaseData || {});
        knowledgeBaseData = kbArray.filter(b => b.id != id); syncRecord('knowledgeBase', knowledgeBaseData);
        try { await db.ref('tpm_system/pdf_files/' + id).remove(); } catch(e){}
        window.renderKnowledgeShelves(); showToast("تم الحذف بنجاح 🗑️");
    }
};

// ==========================================
// 👷‍♂️ محرك بوابة الصيانة الذاتية (JH Portal Engine) والمراجعات
// ==========================================
...
const factoryCLITData = [
  ...
  {"id": "CLIT_10", "region": "الهد", "part": "وصلات الايزو والبولو", "operation": "فحص", "action": "وصلات الايزو والبولو والهيدروليك", "frequency": "أسبوعي", "tools": "N فحص بالعين", "machineState": "تعمل", "optimalState": "الخراطيم جميعها سليم", "degradation": "تسريب في احد الخراطيم او انقطاعة وبالتالي توقف الحقن"},
  {"id": "CLIT_11", "region": "عربة الهيد", "part": "الموتور الخاص بالحركة في اتجاة X", "operation": "تنطيف", "action": "تنظيف الموتور الخاص بالحركة في اتجاة X", "frequency": "أسبوعي", "tools": "بالهواد المضغوط وقطعة قماش", "machineState": "لا تعمل", "optimalState": "الموتور نظيف", "degradation": "تراكم الاوساخ يودي الي ارتفاع حرارة الموتور او تلفة"},
  {"id": "CLIT_12", "region": "عربة الهيد", "part": "الموتور الخاص بالحركة في اتجاةY", "operation": "تنطيف", "action": "تنظيف الموتور الخاص بالحركة في اتجاة Y", "frequency": "أسبوعي", "tools": "بالهواد المضغوط وقطعة قماش", "machineState": "لا تعمل", "optimalState": "الموتور نظيف", "degradation": "تراكم الاوساخ يودي الي ارتفاع حرارة الموتور او تلفة"},
  {"id": "CLIT_13", "region": "عربة الهيد", "part": "موتور عربة الهد اتجاة X", "operation": "فحص", "action": "فحص صوت موتور عربة الهد اتجاة X", "frequency": "شهري", "tools": "بالاذن", "machineState": "تعمل", "optimalState": "صوت الموتور طبيعى", "degradation": "تلف البلى وتلف الملفات الداخلية للموتور"},
  {"id": "CLIT_14", "region": "عربة الهيد", "part": "موتور عربة الهد اتجاة Y", "operation": "فحص", "action": "فحص صوت موتور عربة الهد اتجاة Y", "frequency": "شهري", "tools": "بالاذن", "machineState": "تعمل", "optimalState": "صوت الموتور طبيعى", "degradation": "تلف البلى وتلف الملفات الداخلية للموتور"},
  {"id": "CLIT_15", "region": "عربة الهيد", "part": "وصلات الهواء الخاصة ببستم هد 1", "operation": "فحص", "action": "التأكد من عدم وجود تسريب في وصلات الهواء", "frequency": "أسبوعي", "tools": "N فحص بالعين والاذن", "machineState": "تعمل", "optimalState": "لا يوجد تسريب", "degradation": "تسريب في البستم ويمكن عدم حركنة"},
  {"id": "CLIT_16", "region": "عربة الهيد", "part": "السير المسنن الخاص بحركة عربة الهد", "operation": "فحص", "action": "فحص السير المسنن الخاص بحركة عربة الهد", "frequency": "شهري", "tools": "N فحص بالعين واليد", "machineState": "تعمل", "optimalState": "السير سليم والحركة منتظمة", "degradation": "يمكن حدوث مشكله في عربة الهيد وحدوث دقره"},
  {"id": "CLIT_17", "region": "عربة الهيد", "part": "ترس والجريدة المسننة لمحور Y", "operation": "فحص", "action": "فحص ترس والجريدة المسننة خاص الموتور", "frequency": "أسبوعي", "tools": "N فحص بالعين", "machineState": "لا تعمل", "optimalState": "الترس والجريدة سليمين", "degradation": "حدوث مشكله في حركة عربة الهيد في محور Y"},
  {"id": "CLIT_18", "region": "بلوكات الهيدروليك", "part": "Stream Distribution", "operation": "فحص", "action": "فحص ال كامات stream الخاص بالهد 1 وتغيرها", "frequency": "شهري", "tools": "N فحص بالعين واليد", "machineState": "تعمل", "optimalState": "الكامة تعمل مع فتح  Stream Distribution", "degradation": "يمكن ان تسبب عدم الحقن في حالة عدم فتحها"},
  {"id": "CLIT_19", "region": "بلوكات الهيدروليك", "part": "Stream Distribution", "operation": "تنظيف", "action": "Stream Distribution تنظيف الخاص بالهد 1", "frequency": "أسبوعي", "tools": "بالهواد المضغوط وقطعة قماش", "machineState": "لا تعمل", "optimalState": "نظيف ولا يعوق الحركة", "degradation": "تراكم الاوساخ قد يسبب خلل في حركتة"},
  {"id": "CLIT_20", "region": "HEATING SYSTEM", "part": "خراطيم التسخين", "operation": "فحص", "action": "مراجعة سلامة خراطيم التسخين وتغيير ما يلزم", "frequency": "يومي", "tools": "N فحص بالعين", "machineState": "تعمل", "optimalState": "الخراطيم جميعها سليم", "degradation": "تلف في احد الخراطيم يودي الي تسريب ونقص حرارة"},
  {"id": "CLIT_21", "region": "HEATING SYSTEM", "part": "وحدة التسخين", "operation": "تنظيف", "action": "تنظيف وحدة التسخين كاملة", "frequency": "أسبوعي", "tools": "بالهواد المضغوط وقطعة قماش", "machineState": "لا تعمل", "optimalState": "الوحدة نظيفة", "degradation": "انخفاض جودة التسخين"},
  {"id": "CLIT_22", "region": "HEATING SYSTEM", "part": "عداد الحرارة", "operation": "فحص", "action": "التحقق من درجة الحرارة", "frequency": "يومي", "tools": "N فحص بالعين", "machineState": "تعمل", "optimalState": "درجة الحرارة طبيعية بين 30:50 درجة", "degradation": "ارتفاع او انخفاض الحرارة يقلل من جودة الحقن"}
];
    ...
    container.innerHTML = deptUsers.slice(0, 3).map((u, idx) => {
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; background:rgba(255,255,255,0.05); padding:8px 10px; border-radius:8px; border-right:3px solid var(--gold);">
            <div style="display:flex; align-items:center; gap:10px;">
                ...
                <img src="${u.avatar || '[https://ui-avatars.com/api/?name='+u.name](https://ui-avatars.com/api/?name='+u.name)}" style="width:24px; height:24px; border-radius:50%;">
...
window.filterCLIT = function(freq, btnEl) {
    ...
    document.querySelectorAll('.clit-filter-btn').forEach(b => { b.classList.remove('active'); b.classList.remove('btn-primary'); b.classList.add('btn-outline'); });
...
window.renderChecklistUI = function() {
    ...
    container.innerHTML = activeChecklistTasks.map((t, idx) => {
        ...
        let tagBadge = t.tagId ? `<div style="margin-top:10px; padding:5px; background:var(--danger); color:white; border-radius:5px; font-size:11px; text-align:center; font-weight:bold; cursor:pointer;" onclick="showScreen('tagsScreen'); renderTags();">🚨 تم إصدار تاج للمشكلة (اضغط للعرض)</div>` : '';
...
// ==========================================
// 📉 محرك التحسين المستمر المطور (KK Pillar Engine)
// ==========================================
let pdcaData = [];
...
window.renderKKDashboard = function() {
    ...
    if (selectedDept !== 'الكل') filteredLosses = registeredLosses.filter(l => l.dept === selectedDept);
    ...
    if (selectedDept !== 'الكل') filteredPDCA = pdcaData.filter(p => p.dept === selectedDept);
            ...
            pdcaContainer.innerHTML = filteredPDCA.reverse().map(p => {
                ...
                <div class="card glass-card" style="padding:15px; border-right:4px solid ${statusColor}; cursor:pointer; transition:0.2s;" onclick="viewPDCADetails('${p.id}')" onmouseover="this.style.transform='scale(1.02)'" onmouseout="this.style.transform='scale(1)'">
                    ...
                    <div class="row-flex" style="align-items:center;">
                        ${controls}
                        <span style="font-size:11px; background:rgba(255,255,255,0.1); padding:3px 8px; border-radius:10px; margin-right:10px; color:var(--gold);">عرض A3 📊</span>
                    </div>
                </div>`;
            }).join('');
        }
...
window.openLossRegistration = function(lossId, lossName) {
    ...
    if(mins && !isNaN(mins) && parseInt(mins) > 0) {
        ...
        let lossObj = {
            id: uniqueNumericId().toString(), lossId: lossId, dept: targetDept, minutes: parsedMins,
            date: new Date().toLocaleDateString('ar-EG'), user: currentUser.name || 'مجهول'
        };
...
// 7. تحديث القوائم المنسدلة للأقسام وتصحيح مشكلة (Null innerHTML)
...
// 8. إصلاح مشكلة renderUserManagement (Null Reference Fix)
const originalRenderUserManagement = window.renderUserManagement;
window.renderUserManagement = function() {
    const container = document.getElementById('usersListContainer');
    if(container) { 
        if(typeof originalRenderUserManagement === 'function') originalRenderUserManagement();
    }
};
