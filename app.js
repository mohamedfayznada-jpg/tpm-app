...
async function scanBarcodeFromImage(event) {
    ...
    try {
        ...
        // الأولوية القصوى لنوع الخامة
        let material = "خامة عامة";
        if (text.toLowerCase().includes("steel")) material = "حديد صلب";
        else if (text.toLowerCase().includes("pvc")) material = "بلاستيك PVC";
        
        document.getElementById('globalSearchInput').value = material;
        document.getElementById('searchResults').innerHTML = `<div class="card success-border"><b>الخامة: ${material}</b><br><small>${text}</small></div>`;
        document.getElementById('searchResults').style.display = 'block';
    } catch(e) { alert("فشل قراءة الباركود"); }
}
